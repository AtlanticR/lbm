...
lbm__gam = function( p, dat, pa ) {
  ...
  if ( exists("lbm_local_model_distanceweighted", p) ) {
    if (p$lbm_local_model_distanceweighted) {
      hmod = try( gam( p$lbm_local_modelformula, data=dat, na.action="na.omit", weights=weights, optimizer=p$lbm_gam_optimizer) )
    } else {
      hmod = try( gam( p$lbm_local_modelformula, data=dat, na.action="na.omit", optimizer=p$lbm_gam_optimizer  ) )
    }
  } else {
      hmod = try( gam( p$lbm_local_modelformula, data=dat, na.action="na.omit", optimizer=c("outer", "bfgs")  ) )
  } 
  ...
  lbm_stats = list( sdTotal=sd(dat[,p$variable$Y], na.rm=T), rsquared=ss$r.sq, ndata=ss$n ) # must be same order as p$statsvars
