...
hivemod = function( p, DATA,  storage.backend="bigmemory.ram", continue=FALSE) {
  ...
  if (continue | exists( "ptr", p) ) {
    ...
    p$savedir = file.path(p$project.root, "hivemod", p$spatial.domain )
    ...
    p$stloc = file.path( p$project.root, "hivemod", p$spatial.domain, "tmp" )
    ...
    if (p$hivemod_local_modelengine=="bayesx")  p$libs = c( p$libs, "R2BayesX" )
    ...
    p = hivemod_parameters(p=p) # fill in parameters with defaults where possible
    p = hivemod_db( p=p, DS="filenames" )
    ...
    if (exists("hivemod_local_modelformula", p))  {
      p$variables$local_all = all.vars( p$hivemod_local_modelformula )
    ...
    if (exists("hivemod_global_modelformula", p)) {
      p$variables$global_all = all.vars( p$hivemod_global_modelformula )
    ...
    othervars = c( )
    if (p$hivemod_local_modelengine == "habitat") othervars = c( )
    ...
    if (exists("hivemod_global_modelengine", p)) {
      ...
      hivemod_db( p=p, DS="global_model.redo", B=DATA$input )
    }
    ...
    sbox = list( 
      plats = seq( p$corners$plat[1], p$corners$plat[2], by=p$hivemod_distance_statsgrid ),
      plons = seq( p$corners$plon[1], p$corners$plon[2], by=p$hivemod_distance_statsgrid ) )
      ...
      Yraw = hivemod_attach( p$storage.backend, p$ptr$Yraw )
      p$qs0 = quantile( Yraw[], probs=p$hivemod_quantile_bounds, na.rm=TRUE  )
     ...
     # default just copy Yraw ... but if covars are modelled then overwrite with residuals (below)
      ...
      if (exists("hivemod_global_modelengine", p)) {
        ...
        covmodel = hivemod_db( p=p, DS="global_model")
        if (!is.null(covmodel)) {
      ...
      p$qs = quantile( Y[], probs=p$hivemod_quantile_bounds, na.rm=TRUE  )


      if (p$hivemod_local_modelengine == "habitat") {
        logitY = hivemod_db( p=p, DS="presence.absense" )
          if (p$storage.backend == "bigmemory.ram" ) {
    ...
    hivemod_db( p=p, DS="statistics.Sflag" )
  ...
  # data locations
    ...
    if ( !exists("hivemod_distance_scale", p)) {
      Yloc = hivemod_attach( p$storage.backend, p$ptr$Yloc )
      p$hivemod_distance_scale = min( diff(range( Yloc[,1]) ), diff(range( Yloc[,2]) ) ) / 10
      message( paste( "Crude distance scale:", p$hivemod_distance_scale ) )
    }
    if ( !exists("hivemod_distance_min", p)) p$hivemod_distance_min = mean( c(p$hivemod_distance_prediction, p$hivemod_distance_scale /20 ) )
    if ( !exists("hivemod_distance_max", p)) p$hivemod_distance_max = mean( c(p$hivemod_distance_prediction*10, p$hivemod_distance_scale * 2 ) )
    ...
    hivemod_db( p=p, DS="save.parameters" )  # save in case a restart is required .. mostly for the pointers to data objects
  ...
  message (p$hivemod_current_status )
  ...
  # 2. redo ,issing solutions with relaxed spatial extent
  p$timei2 =  Sys.time()
  o = hivemod_db(p=p, DS="statistics.status.reset" ) 
  if (length(o$todo) > 0) {
  ...
  if (resp=="Yes") {
    ...
    message( "You can delete them by running: hivemod_db( p=p, DS='cleanup' ), once you are done.")
  }
