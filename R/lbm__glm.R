...
lbm__glm = function( p, x, pa ) {
  ...
  if ( exists("lbm_local_model_distanceweighted", p) ) {
    if (p$lbm_local_model_distanceweighted) {
      hmod = try( glm( p$lbm_local_modelformula, data=x, weights=Y_wgt  ) ) 
    } else {
      hmod = try( glm( p$lbm_local_modelformula, data=x  ) ) 
    }
  } else {
      hmod = try( glm( p$lbm_local_modelformula, data=x  ) ) 
  } 
  ...
  lbm_stats = list( sdTotal=sd(x[,p$variable$Y], na.rm=T), rsquared=rsq, ndata=nrow(x) ) # must be same order as p$statsvars .. pseudo rsquared for logistic .. for poisson {1- logLik(mod) / logLik(mod_saturated)} might be better
