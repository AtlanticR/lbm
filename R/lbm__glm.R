...
lbm__glm = function( p, dat, pa ) {
  ...
  if ( exists("lbm_local_model_distanceweighted", p) ) {
    if (p$lbm_local_model_distanceweighted) {
      hmod = try( glm( p$lbm_local_modelformula, data=dat, weights=Y_wgt  ) ) 
    } else {
      hmod = try( glm( p$lbm_local_modelformula, data=dat  ) ) 
    }
  } else {
      hmod = try( glm( p$lbm_local_modelformula, data=dat  ) ) 
  } 
  ...
  lbm_stats = list( sdTotal=sd(dat[,p$variable$Y], na.rm=T), rsquared=rsq, ndata=nrow(dat) ) # must be same order as p$statsvars .. pseudo rsquared for logistic .. for poisson {1- logLik(mod) / logLik(mod_saturated)} might be better
