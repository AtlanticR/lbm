  ...
  lbm_plot = function( p, obj, RES=NULL, MESH=NULL, SPDE=NULL, vname=NULL, idat=NULL, nxout=100, nyout=100 ) {
    #\\ Simple diagnostic plots for lbm statistics and predictions
    #\\   lbm_plot( p=p, odb="mesh" ) :: mesh, range, nugget, partial.sill, kappa, intercept, etc
    ...
    pp = grep("datalocation", obj )
      if ( length(pp) > 0 ) {
        ...
        Y0 = lbm_attach( p$storage.backend, p$ptr$Y0 )
    ...
    if ("map.prediction" %in% obj) {
      ...
      posterior = p$lbm_local_family()$linkinv( posterior )   # return to original scale
