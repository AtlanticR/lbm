...
hivemod__habitat = function( p, x, pa ) {
   #\\ this is the core engine of hivemod .. localised space-time habiat modelling
  ...
  if ( exists("hivemod_local_model_distanceweighted", p) ) {
    if (p$hivemod_local_model_distanceweighted) {
      Hmodel = try( gam( p$hivemod_local_modelformula, data=x, family=binomial, weights=weights, optimizer=c("outer","optim")  ) )
    } else {
      Hmodel = try( gam( p$hivemod_local_modelformula, data=x, family=binomial, optimizer=c("outer","optim")  ) )
    }
  } else {
      Hmodel = try( gam( p$hivemod_local_modelformula, data=x, family=binomial ) )
  } 
  ...
  if ( exists("hivemod_local_model_distanceweighted", p) ) {
    if (p$hivemod_local_model_distanceweighted) {
      Amodel = try( gam( p$hivemod_local_modelformula, data=x, weights=weights, optimizer=c("outer","optim")  ) )
    } else {
      Amodel = try( gam( p$hivemod_local_modelformula, data=x, optimizer=c("outer","optim")  ) )
    }
  } else {
      Amodel = try( gam( p$hivemod_local_modelformula, data=x ) )
  } 
  ...
  ss = summary(hmod)
  hivemod_stats = list( sdTotal=sd(Y[], na.rm=T), rsquared=rsq, ndata=nrow(x) ) # must be same order as p$statsvars
