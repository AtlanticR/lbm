...
hivemod_interpolate_xy_simple = function( interp.method, data, locsout, datagrid=NULL,
  ...
  # identity links by default .. add more if needed here
    ...
    MESH = hivemod_mesh_inla( locs, lengthscale=lengthscale )
    ...
    RES = NULL
    RES = hivemod_inla_call( FM=FM, DATA=DATA, SPDE=SPDE, FAMILY="gaussian" )
  ...
  if (interp.method=="spBayes"){
    ...
    stv = hivemod_variogram( xy, z, methods=method )
