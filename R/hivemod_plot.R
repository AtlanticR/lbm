  ...
  hivemod_plot = function( p, obj, RES=NULL, MESH=NULL, SPDE=NULL, vname=NULL, idat=NULL, nxout=100, nyout=100 ) {
    #\\ Simple diagnostic plots for hivemod statistics and predictions
    #\\   hivemod_plot( p=p, odb="mesh" ) :: mesh, range, nugget, partial.sill, kappa, intercept, etc
    ...
    pp = grep("datalocation", obj )
      if ( length(pp) > 0 ) {
        ...
        Y0 = hivemod_attach( p$storage.backend, p$ptr$Y0 )
    ...
    if ("map.prediction" %in% obj) {
      ...
      posterior = p$hivemod_local_family()$linkinv( posterior )   # return to original scale
