...
hivemod__inla = function( p, x, pa ) {
  ...
  if ( !exists("inla.mesh.max.edge", p))  p$inla.mesh.max.edge = c(  0.025,   0.04 )    # proportion of 2*p$hivemod_distance_scale or equivalent: c(inside,outside) -- must be positive valued
  if ( !exists("inla.mesh.offset", p))  p$inla.mesh.offset   = c( - 0.025,  - 0.05 )   # how much to extend inside and outside of boundary: proportion of hivemod_distance_scale .. neg val = proportion
  if ( !exists("inla.mesh.cutoff", p)) p$inla.mesh.cutoff   = c( - 0.05,   - 0.5 )    ## min distance allowed between points: proportion of hivemod_distance_scale ; neg val = proportion
  ...
  if ( !exists("hivemod_noise", p)) p$hivemod_noise = p$pres / 10  # add a little noise to coordinates to prevent a race condition
  ...
  #-----------------
  # row, col indices
  if ( !exists("hivemod.posterior.extract", p)) {
  ...
  locs_noise = runif( nrow(x)*2, min=-p$pres*p$hivemod_noise, max=p$pres*p$hivemod_noise ) # add  noise  to prevent a race condition

  # also sending direct distances rather than proportion seems to cause issues..
  MESH = hivemod_mesh_inla( locs=x[,p$variables$LOC] + locs_noise,
    # lengthscale=p$hivemod_distance_prediction*2,  
    # max.edge=p$inla.mesh.max.edge * p$hivemod_distance_prediction*2,
  ...
  if (0)  plot( MESH )  # or ... hivemod_plot( p=p, "mesh", MESH=MESH )
  ...
  if (!exists("hivemod_local_modelformula", p) )  p$hivemod_local_modelformula = formula( z ~ -1 + intercept + f( spatial.field, model=SPDE ) ) # SPDE is the spatial covariance model .. defined in 

  RES = NULL
  RES = hivemod_inla_call( FM=p$hivemod_local_modelformula, DATA=DATA, SPDE=SPDE, FAMILY=p$inla_family )
  ...
  preds = NULL
  if (! exists("hivemod_inla_prediction", p) ) p$hivemod_inla_prediction="direct"

  if ( p$hivemod_inla_prediction=="direct" ) {
  ...
  if ( p$hivemod_inla_prediction=="projected") {
    ...
    posterior = sapply( posterior.samples, p$hivemod.posterior.extract, rnm=rnm )
...
# bathymetry.figures( DS="predictions", p=p ) # to debug
  inla.summary = hivemod_summary_inla_spde2 ( RES, SPDE )
