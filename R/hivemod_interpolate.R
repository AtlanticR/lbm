...
hivemod_interpolate = function( ip=NULL, p ) {
  ...
  #---------------------
  # data for modelling
  # dependent vars # already link-transformed in hivemod_db("dependent")
  ...
  Pn = hivemod_attach( p$storage.backend, p$ptr$Pn )
  ...
  if ( exists("TIME", p$variables) ) {
    Ytime = hivemod_attach( p$storage.backend, p$ptr$Ytime )
  }
  ...
  if (p$hivemod_local_modelengine=="habitat") {
    Ylogit = hivemod_attach( p$storage.backend, p$ptr$Ylogit )
    Plogit = hivemod_attach( p$storage.backend, p$ptr$Plogit )
    Plogitsd = hivemod_attach( p$storage.backend, p$ptr$Plogitsd )
  }

  Yi = hivemod_attach( p$storage.backend, p$ptr$Yi )
  ...
  upsampling = upsampling[ which(upsampling*p$hivemod_distance_scale <= p$hivemod_distance_max )]
...
# main loop over each output location in S (stats output locations)
  for ( iip in ip ) {
      ...
      if ( ndata < p$n.min )  {
        ...
        if ( ndata <= p$n.max * 1.5 ) { # if close to p$n.max, subsample quickly 
          ...
          for ( dsamp in downsampling )  { # lots of data .. downsample
            ...
            if ( hivemod_distance_cur <= p$hivemod_distance_min ) {
              # reached lower limit in distance, taking a subsample instead
              U = which( dlon < p$hivemod_distance_min & dlat < p$hivemod_distance_min ) # faster to take a block 
              U = U[ .Internal( sample( length(U), p$n.max, replace=FALSE, prob=NULL)) ]
              ndata = length(U)
              break()
            }
    ...
    # dist_model = hivemod_distance_cur

    dist_prediction = min( p$hivemod_distance_prediction, hivemod_distance_cur ) # do not predict greater than p$hivemod_distance_prediction
    ...
    if (exists("local_cov", p$variables)) {
      ...
      for (ci in 1:length(p$variables$local_cov)) {
        ...
        pu = NULL
        pu = hivemod_attach( p$storage.backend, p$ptr$Pcov[[vn]] )
        nts = ncol(pu)
        if ( nts== 1 ) {
          pvars = c( pvars, vn )
    ...
    dat[, p$variables$Y] = p$hivemod_local_family$linkfun ( dat[, p$variables$Y] ) 
    if (p$hivemod_local_modelengine=="habitat") {
      dat[, p$variables$Ylogit ] = p$hivemod_local_family_logit$linkfun ( dat[, p$variables$Ylogit] ) ### -- need to conform with data structure ... check once ready
    }
    dat$plon = Yloc[YiU,1]
    dat$plat = Yloc[YiU,2]
  ...
  # use a larger data grid to interpolate.. right now too slow to use so skip this step
    ...
    gc()
    res =NULL
    res = try( switch( p$hivemod_local_modelengine, 
    ...
    if (p$hivemod_local_modelengine=="habitat") {
      res$predictions$logitmean = p$hivemod_local_family_logit$linkinv( res$predictions$logitmean )
      res$predictions$logitsd   = p$hivemod_local_family_logit$linkinv( res$predictions$logitsd )
    }
 
    if (exists( "hivemod_quantile_bounds", p)) {
      tq = quantile( Y[YiU], probs=p$hivemod_quantile_bounds, na.rm=TRUE  )
    ...
    # stats collator
    if (!exists("hivemod_stats",  res) ) res$hivemod_stats = list()
    
    if (!exists("sdSpatial", res$hivemod_stats)) {
      ...
      res$hivemod_stats["sdSpatial"] = NA 
      res$hivemod_stats["sdObs"] = NA 
      res$hivemod_stats["range"] = NA
      res$hivemod_stats["phi"] = NA
      res$hivemod_stats["nu"] = NA
      if ( !is.null(ores)) {
        res$hivemod_stats["sdSpatial"] = sqrt( ores[["varSpatial"]] ) 
        res$hivemod_stats["sdObs"] = sqrt(ores[["varObs"]]) 
        res$hivemod_stats["range"] = ores[["range"]]
        res$hivemod_stats["phi"] = ores[["phi"]]
        res$hivemod_stats["nu"] = ores[["nu"]]
      } 
    }
    ...
    if ( exists("TIME", p$variables) ){
      ...
      res$hivemod_stats["ar_timerange"] = NA 
      res$hivemod_stats["ar_1"] = NA
      ...
      if (length(pac_i) > 5) {
        ...
        if (length(piid) > 5 ) {
          ts.stat = NULL
          ts.stat = try( hivemod_timeseries( pac$mean, method="fft" ) )
          if (!is.null(ts.stat) && !inherits(ts.stat, "try-error") ) {
            res$hivemod_stats["ar_timerange"] = ts.stat$quantilePeriod 
            if (all( is.finite(pac$mean))) {
              ...
              if (length(afin) > 5 && var( pac$mean, na.rm=TRUE) > p$eps ) {
                ...
                if (!inherits(ar1, "try-error")) {
                  if ( length(ar1$ar) == 1 ) {
                    res$hivemod_stats["ar_1"] = ar1$ar
                  }  
                } 
              }
            }
            if ( !is.finite(res$hivemod_stats[["ar_1"]]) ) {
              ar1 = try( cor( pac$mean[1:(length(piid) - 1)], pac$mean[2:(length(piid))], use="pairwise.complete.obs" ) )
              if (!inherits(ar1, "try-error")) res$hivemod_stats["ar_1"] = ar1 
            }
          } 

          ### Do the logistic model here ! -- if not already done ..
          if (!exists("ts_K", res$hivemod_stats)) {
    ...
    for ( k in 1: length(p$statsvars) ) {
      if (exists( p$statsvars[k], res$hivemod_stats )) {
        S[Si,k] = res$hivemod_stats[[ p$statsvars[k] ]]
      }
    }
