...
hivemod__LaplacesDemon = function( p, x, pa ) {
   #\\ this is the core engine of hivemod .. localised space-time habiat modelling
    ...
    if ( exists("hivemod_local_model_distanceweighted", p) ) {
      if (p$hivemod_local_model_distanceweighted) {
        Hmodel = try( gam( p$hivemod_local_modelformula, data=x, weights=weights, optimizer=c("outer","optim")  ) )
      } else {
        Hmodel = try( gam( p$hivemod_local_modelformula, data=x, optimizer=c("outer","optim")  ) )
      }
    } else {
        Hmodel = try( gam( p$hivemod_local_modelformula, data=x ) )
    } 
    ...
    if (exists( "hivemod_quantile_bounds", p)) {
      tq = quantile( x[,p$variables$Y], probs=p$hivemod_quantile_bounds, na.rm=TRUE  )
    ...
    ss = summary(Hmodel)
    hivemod_stats = list( sdTotal=sd(Y[], na.rm=T), rsquared=ss$r.sq, ndata=ss$n ) # must be same order as p$statsvars

    return( list( predictions=newdata, hivemod_stats=hivemod_stats ) )
